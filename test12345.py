...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.textbox_data.setPlaceholderText("Choose Data (.txt)")
        ...
        self.label_top_speed_1.setText("Top Speed")
        ...
        self.label_top_speed_2.setStyleSheet('''
                                            #label_top_speed_2 {
                                                font-weight: 600;
                                            }
                                            ''')
        self.label_top_speed_2.setObjectName("label_top_speed_2")
        self.label_top_speed_2.setText("000 km/h")
    ...
    def check_crash(self, event):
        cur_time, cur_dist, cur_speed, cur_lat, cur_long = event
        if cur_dist == 0 and cur_speed >= 10:
            return True

        return False

    def check_near_crash(self, event):
        cur_time, cur_dist, cur_speed, cur_lat, cur_long = event
        if cur_dist == 1 and cur_speed >= 10:
            return True

        return False

    def check_exists(self, check, events):
        for event in events:
            for i in check:
                if i in event:
                    return True
    ...
    def process_data(self):
        ...
        start = lines[0].split()[0] + " " + lines[0].split()[1]
        end = lines[-1].split()[0] + " " + lines[0].split()[1]
        ...
        for line in lines:
            ...
            distance = int(parts[2]) # meter
            speed = int(parts[3]) # km/h
            latitude = float(parts[4])
            longitude = float(parts[5])
            
            event_time = datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")
            
            events.append((event_time, distance, speed, latitude, longitude))
        
        self.raw_events = events
        
        all_events = {"near_crash": [], "crash": []}
        top_speed = events[0][2]
        bot_speed = events[0][2]

        for i in range(len(events)):
            current_event = events[i]
            
            if (current_event[2] > top_speed):
                top_speed = current_event[2]
            
            if (current_event[2] < bot_speed):
                bot_speed = current_event[2]
            
            if self.check_near_crash(current_event):
                if i == 0 and i != len(events) - 1: # at the beginning
                    all_events["near_crash"].append([current_event, events[i + 1], "Near Crash"])
                elif i == len(events) - 1 and i != 0: # at the end
                    # check if events[i - 1], current_event are there in the list, if not append
                    if not self.check_exists([events[i - 1], current_event], all_events["near_crash"]):
                        all_events["near_crash"].append([events[i - 1], current_event, "Near Crash"])
                else:
                    # check if events[i - 1], current_event, events[i + 1] are there in the list, if not append
                    if not self.check_exists([events[i - 1], current_event], all_events["near_crash"]):
                        all_events["near_crash"].append([events[i - 1], current_event, events[i + 1], "Near Crash"])
            
            elif self.check_crash(current_event):
                if i == 0 and i != len(events) - 1:  # at the beginning
                    all_events["crash"].append([current_event, events[i + 1], "Crash"])
                elif i == len(events) - 1 and i != 0: # at the end
                    # check if events[i - 1], current_event are there in the list, if not append
                    if not self.check_exists([events[i - 1], current_event], all_events["crash"]):
                        all_events["crash"].append([events[i - 1], current_event, "Crash"])
                else:
                    # check if events[i - 1], current_event, events[i + 1] are there in the list, if not append
                    if not self.check_exists([events[i - 1], current_event], all_events["crash"]):
                        all_events["crash"].append([events[i - 1], current_event, events[i + 1], "Crash"])
            
        return all_events, top_speed, bot_speed
    
    def display_data(self, data, top_speed, bot_speed):
        length = len(data["near_crash"]) + len(data["crash"])
        self.label_total_2.setText(str(length))
        self.label_near_2.setText(str(len(data["near_crash"])))
        self.label_crash_2.setText(str(len(data["crash"])))
        self.label_top_speed_2.setText(f"{top_speed:03d} km/h")
        self.label_bot_speed_2.setText(f"{bot_speed:03d} km/h")
        
        combined_events = data["crash"] + data["near_crash"]
        sorted_events = sorted(combined_events, key=lambda event: event[0][0])
        ...
        for i in range(length):
            ...
            self.label_time.setText(sorted_events[i][0][0].strftime("%H:%M:%S"))
            ...
            self.label_date.setText(sorted_events[i][0][0].strftime("%Y-%m-%d"))
    ...
    def data_clicked(self, event, data_name):
        ...
        if obj_list:
            ...
            start_time = data[0][0] - self.start_time - timedelta(seconds=1)
            end_time = data[-2][0] - self.end_time
            ...
            # display data
            self.label_speed_2.setText(f"{data[0][2]:03d} km/h")
            self.label_lat_2.setText(f"{data[0][3]:7.7}")
            self.label_long_2.setText(f"{data[0][4]:7.7}")
            ...
            self.update_map(data[0][3], data[0][4])
    ...
    def updateState(self, position):
        ...
        if (0 <= index < len(self.data_state)):
            start_time = self.data_state[index][0] - self.start_time - timedelta(seconds=1)
            
            self.label_speed_2.setText(f"{self.data_state[index][2]:03d} km/h")
            self.label_lat_2.setText(f"{self.data_state[index][3]:7.7}")
            self.label_long_2.setText(f"{self.data_state[index][4]:7.7}")
            ...
            self.update_map(self.data_state[index][3], self.data_state[index][4])
    ...
    def submit(self):
        ...
        all_events, top_speed, bot_speed = self.process_data()
        self.display_data(all_events, top_speed, bot_speed)
        
        fileName = self.textbox_vid_2.toPlainText()
        ...
        while True:
            ...
            if (current_time_s >= 60 + count_minute * 60) or (current_time_s == 0 and len(self.time_list) != 0):
                ...
                tanggal = (self.start_time + timedelta(seconds=seconds)).strftime("%Y-%m-%d")
                ...
                avg_distance = self.average([item[1] for item in self.raw_events], count_minute)
                category_distance = 0
                avg_speed = self.average([item[2] for item in self.raw_events], count_minute)
                category_speed = 0
                
                print(tanggal, waktu, closing_time_avg, reopening_time_avg, closed_time_avg, blink_duration, blink_frequency, microsleep, perclos, avg_distance, category_distance, avg_speed, category_speed)
                
                IO.write_data_to_csv([tanggal, waktu, closing_time, reopening_time, closed_time, blink_duration, blink_frequency, microsleep, perclos], csv_file)
